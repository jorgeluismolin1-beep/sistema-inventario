...
<?php
    ...
    // Mostrar sta (0)s.
    $sqlquery = "SELECT * FROM activos WHERE status=0 ORDER BY descripcion";
    ...
    if ($results && mysqli_num_rows($results) > 0) {
        while ($fila = mysqli_fetch_assoc($results)) {
            ...
            echo "<div class='col-2'>0</div>";
            echo "<div class='col-2'>
             <a href='habilitar_activos.php?ID_Activos=$videntificador'>
                        <img src='assets/img/Desabilitar.png' width='30' height='30' title='Deshabilitar'>
                    </a>
                  </div>";
            echo "</div>";
        }
    } else {
        echo "<div class='row mt-2'><div class='col'>No hay activos deshabilitados.</div></div>";
    }
